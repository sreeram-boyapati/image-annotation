...
Mat multi_channelSegmentation(Mat src){
    ...
    int numOfSegments = 0;
    //Apply watershed
    Mat segments = watershedSegment(src,numOfSegments);
    //Merge segments in order to reduce over segmentation
    mergeSegments(src,segments, numOfSegments);
    //To display the merged segments
    Mat wshed = createSegmentationDisplay(segments, numOfSegments, src);
    //To display the merged segments blended with the image
    //Mat wshedWithImage = createSegmentationDisplay(segments, numOfSegments, src);
    return wshed;
